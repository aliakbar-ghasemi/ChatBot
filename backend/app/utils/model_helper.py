...
from app.utils.text_helper_mixed_lang import summarize_text
import json

# AI model categories mapped to the best models
MODEL_CATEGORIES = [
    {"id": 1, "category": "General Language Understanding", "model": "llama3:8b"},
    {
        "id": 2,
        "category": "Conversational AI (Chatbots, Assistants)",
        "model": "llama3:8b",
    },
    {"id": 3, "category": "English Learning & Fluency", "model": "mistral:7b"},
    {"id": 4, "category": "Python Development", "model": "deepseek-coder:6.7b"},
    {
        "id": 5,
    ...
    {"id": 8, "category": "Data Science & AI Development", "model": "mistral:7b"},
    {"id": 9, "category": "Mathematical & Logical Reasoning", "model": "mistral:7b"},
    {"id": 10, "category": "Creative Writing & Storytelling", "model": "mistral:7b"},
    {"id": 11, "category": "Scientific & Research Analysis", "model": "llama3:8b"},
    {"id": 12, "category": "Medical & Healthcare AI", "model": "meditron:7b"},
    {"id": 13, "category": "Finance, Business & Economics", "model": "gemma:7b"},
    {"id": 14, "category": "Cybersecurity & Threat Detection", "model": "falcon:7b"},
]
...
def classify_prompt(user_prompt):
    ...
    try:
        ...
        logger.debug(f"🧠 Category Detected: {parsed_category}")
...
Summarize the the following text in **shortest** and **fastest** way possible.
  
- **Do not reply any questions just summarize**  
- **Keep key points only**  
- **If the text includes code, ignore the code**  
- **If the text is in Persian, provide a brief summary in Persian**  
- **Output should be under 2 sentences and maximum 200 characters**

{user_prompt}  
"""


def summarize_prompt(user_prompt):
    try:
        response = ollama.chat(
            # model="mistral:7b",
            model="gemma:2b",
            ...
            messages=[
                ...
                {
                    "role": "user",
                    "content": Summarize_System_Prompt.format(user_prompt=user_prompt),
                },
            ],
            options={"temperature": 0},  # Ensure deterministic output
        )
...
def select_model(user_prompt: str) -> str:
    ...
    logger.debug(f"🚀 select_model: Routing user prompt to the best AI model...")
    ...
    # summary = summarize_prompt(user_prompt)
    selected_model = classify_prompt(user_prompt)
    logger.debug(f"🚀 Routing to model: {selected_model}")
